...
// ** save clip
  $("#detaiSave").click(function(evt){
    ...
    contentContainer.children().each(function(){
      console.info(contentContainer.children());
...
/*
    contentContainer.children("sound").each(function(){
      var _sound = $(this).sound;
      if(!_sound){
	$(this).remove();
      } else {
	var prefix = client.URL.HOST_URL + client.SYMBOL.SLASH + client.URL.BASE_URL + client.GLOBAL_CACHE["userInfo"].name + "/clip/sound";
	if(sound.indexOf(prefix) != -1){
	  id = sound.split(prefix);
	  sound = id[1];
	}
	_data.note.push({sound:sound});
	console.info(_data.note);
      }
    });
*/
    ...
    RequestUtil.postFunc({
      url:client.URL.HOST_URL + client.SYMBOL.SLASH + client.URL.BASE_URL +"my/clip.json",
      data:_data,
