ClipDetailWidget = function(_container,options){
  ...
  var _view = Backbone.View.extend({
	...
	if(model.content[i].image && !isNaN(model.content[i].image)){
	  model.content[i].image = client.URL.HOST_URL + client.SYMBOL.SLASH + client.URL.BASE_URL + "my/clip/image/"+model.content[i].image;
	  // console.info(model.content[i].image);
	}
      }
      for(var j=0;j<model.note.length;j++){
	if(model.note[j].sound && !isNaN(model.note[j].sound)){
	  model.note[j].sound = client.URL.HOST_URL + client.SYMBOL.SLASH + client.URL.BASE_URL +"my/clip/sound/"+model.note[j].sound;
	  // console.info(model.note[j].sound);
	}
      }
	...
	$("#contentWrapper").animate({"width":0,"opacity":0},"slow","swing",function(){
	  ...
	  $("#localImg").bind("click",function(evt){
	    ...
	    var actionUrl = client.URL.HOST_URL + client.SYMBOL.SLASH + client.URL.BASE_URL +"my/clip/image";
		...
		if(returnVal != null && returnVal != ""){
		  ...
		  if(returnObj[0] == 0){
		    ...
		    for(var i=0;i<imgids.length;i++){
		      var url = client.URL.HOST_URL + client.SYMBOL.SLASH + client.URL.BASE_URL +"my/clip/image/" +imgids[i];
		      var img = $("<img class='detail-image' src= "+url+">");
		      contentContainer.append(img);
		    }
		  }
	...
	if(_text){//&& text.replace(/(^\s*)|(\s*$)/g,"") != ""){
	  ...
	  var prefix = client.URL.HOST_URL + client.SYMBOL.SLASH + client.URL.BASE_URL + "my/clip/image/";
    ...
    RequestUtil.postFunc({
      url:client.URL.HOST_URL + client.SYMBOL.SLASH + client.URL.BASE_URL + "my/clip/"+view.id,
      data:_data,
