ClipWidget = function(_container,options){
  ...
  var _view = Backbone.View.extend({
	...
	collection = this.previewList.toJSON();
      ...
      // console.info("ClipWidget : ");
      for(var i=0;i<collection.length;i++){
	if(collection[i].content.image && !isNaN(collection[i].content.image)){
	  //collection[i].content.image = client.URL.HOST_URL + client.SYMBOL.SLASH + client.URL.BASE_URL + client.GLOBAL_CACHE["userInfo"].name+"/clip/image/"+collection[i].content.image+"/128";
	  collection[i].content.image = client.URL.HOST_URL + client.SYMBOL.SLASH + client.URL.BASE_URL + "my/clip/image/"+collection[i].content.image;
	}
    ...
    iniClipList:function(){
      ...
      this.previewList.url = client.URL.HOST_URL + client.SYMBOL.SLASH + client.URL.BASE_URL + "my/clip/all";
