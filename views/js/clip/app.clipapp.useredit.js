App.ClipApp.UserEdit = (function(App, Backbone, $){
  ...
  var FaceModel = App.Model.extend({
    url:function(){
      return App.ClipApp.encodeURI(P+"/"+App.ClipApp.getMyUid()+"/face");
    }
  });
  var PassEditModel = App.Model.extend({
    url:function(){
      return App.ClipApp.encodeURI(P+"/"+App.ClipApp.getMyUid()+"/passwd");
    },
  ...
  var EmailEditModel = App.Model.extend({
    url:function(){
      if(this.get("address")){
	return App.ClipApp.encodeURI(P+"/"+App.ClipApp.getMyUid()+"/email/"+this.get("address"));
      }
      return App.ClipApp.encodeURI(P+"/"+App.ClipApp.getMyUid()+"/email");
    }
  });
  ...
  var PassView = App.ItemView.extend({
	...
	model.save({lang:lang},{
	  type:'PUT',
	  url : App.ClipApp.encodeURI(P+"/"+App.ClipApp.getMyUid()+"/lang"),
	  success:function(model,res){
